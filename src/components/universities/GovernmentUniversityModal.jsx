export default function GovernmentUniversityModal({ selected, setSelected }) {
      ...
      <div className="bg-white rounded-3xl shadow-modal w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-4 gap-3">
            {[
              { l: "NIRF Rank", v: `#${selected.nirf}` },
              { l: "NAAC", v: selected.naac },
              { l: "UG Fees/yr", v: selected.fees.ug },
              { l: "PG Fees/yr", v: selected.fees.pg },
            ].map((st) => (
          ...
          <div className="bg-brand-50 border border-brand-200 rounded-xl p-4">
            <span className="text-sm text-brand-700">⭐ Known for: </span>
            <span className="text-sm font-semibold text-brand-900">{selected.knownFor}</span>
          </div>
          ...
          {/* Section: Admission */}
          <div>
            <h4 className="text-sm font-semibold text-gray-700 mb-3 uppercase tracking-wide">🎯 How to Get Admission</h4>
          ...
          {/* Section: Highlights */}
          <div>
            <h4 className="text-sm font-semibold text-gray-700 mb-2 uppercase tracking-wide">✨ Highlights</h4>
            <div className="flex flex-wrap gap-2">
              {selected.highlights.map((h) => <span key={h} className="pill-green">{h}</span>)}
            </div>
          </div>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <a href={selected.applyLink} target="_blank" rel="noopener noreferrer" className="btn-primary flex-1 text-center text-sm">Apply / Admission →</a>
          </div>
